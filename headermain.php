 <nav class="template-main-menu">
                  <ul class="menu">
                  
                     <strong> <a href="index.php">Home</a></strong>
                   
				   
				   
                    <li class="menu-item menu-item-has-children">
                      <a href="">About Us</a>
                      <ul class="sub-menu">
					  
					  <li class="menu-item">
                          <a href="about.php">About RCN Missionary School</a>
                        </li>
                        <li class="menu-item">
                          <a href="vision.php">Vision & Mission</a>
                        </li>
                        <li class="menu-item">
                          <a href="provost.php">Office of the Provost</a>
                        </li>
                       
                     
                     
                      </ul>
                    </li>
                    <li class="menu-item menu-item-has-children">
                      <a href="academics.php">Academics</a>
                      <ul class="sub-menu">
                        <li class="menu-item">
                          <a href="explore.php">Explore Programs</a>
                        </li>
                        <li class="menu-item">
                          <a href="pgdt.php">Post Graduate Diploma Training(PGDT)</a>
                        </li>
                        <li class="menu-item">
                          <a href="diploma.php">Diploma program </a>
                        </li>
                        <li class="menu-item">
                          <a href="tuition.php">Tuition Fees</a>
                        </li>
						            <li class="menu-item">
                          <a href="https://admin.adullam.ng/#/platform/practicum">Practicum Log book</a>
                        </li>
                       
                      </ul>
                    </li>
                    <li class="menu-item menu-item-has-children">
                      <a href="">Admissions</a>
                      <ul class="sub-menu">
                        <li class="menu-item">
                          <a href="https://admin.adullam.ng/platform/sign-up/register">Apply Now</a>
                        </li>
                        <li class="menu-item">
                          <a href="financialaid.php">Scholarship</a>
                        </li>
                        
						 <li class="menu-item">
                          <a href="calender.php">Academic Calender</a>
                        </li>
						
						<li class="menu-item">
                          <a href="health.php">Health Insurance </a>
                        </li>
                      </ul>
                    </li>
                    
                    <li class="menu-item menu-item-has-children">
                      <a href="">Life at RCN theology</a>
                      <ul class="sub-menu">
                        
                        <li class="menu-item">
                          <a href="chapel.php">The Chapel </a>
                        </li>
                        <li class="menu-item">
                          <a href="embassy.php">RCN Embassy</a>
                        </li>
						
						
						<li class="menu-item">
                          <a href="hostel.php">Housing & Hostels</a>
                        </li>
                      </ul>
                    </li>
				   
				   
				   <li class="menu-item">
                      <a href="partnership.php">Partnership
					  </a>
                    </li>
					<li class="menu-item">
                      <a href="contact.php">Contact us</a>
                    </li>
                      <li class="menu-item">
                      <a href="https://seminary.adullam.ng/main/login_form.php">Student Portal</a>
                    </li>
                  </ul>
                </nav>